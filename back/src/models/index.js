...
const db = {};
const sequelize = new Sequelize(config.db.database, config.db.user, config.db.password, config.db.options); // creation of the obj sequelize --> making the connection with the config

const folderModels = ['IoT', 'Room', 'Scenario', 'User'];
            ...
            if (!isDirModels(file)) {
                fs.readdirSync(path.join(__dirname, file))
                    ...
                    .forEach((tmp) => {
                        ...
                        const model = require(path.join(path.join(__dirname, file), tmp))(sequelize, Sequelize.DataTypes);
                        db[model.name] = model;
                    });
                break;
            }
